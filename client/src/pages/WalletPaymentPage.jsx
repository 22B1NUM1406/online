...
import { ArrowLeft, CheckCircle, Clock, RefreshCw, AlertCircle, Wallet } from 'lucide-react';
import { checkWalletQPayPayment } from '../services/api';
...
const WalletPaymentPage = () => {
  ...
  // Get QPay data from WalletPage
  const { qrImage, qrText, urls, invoiceId, amount, type } = location.state || {};
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (paymentStatus !== 'paid') {
        setNotification({
          message: 'Төлбөр баталгаажих хугацаа дууслаа. Хэтэвч хэсэгт очиж шалгана уу.',
          type: 'warning'
        });
      }
    }, 600000);
  ...
  const checkPayment = async () => {
    try {
      ...
      const response = await checkWalletQPayPayment(invoiceId);
      ...
      if (response.success && response.data.paid) {
        ...
        if (response.data.newBalance) {
          updateWallet(response.data.newBalance);
        }
        
        setNotification({ 
          message: `${formatPrice(amount)} амжилттай цэнэглэгдлээ!`, 
          type: 'success' 
        });
        ...
        {paymentStatus === 'paid' && (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-2">
              Амжилттай цэнэглэгдлээ!
            </h2>
            <p className="text-gray-600 mb-2">
              Таны хэтэвч {formatPrice(amount)} нэмэгдлээ
            </p>
            <div className="mt-6 p-4 bg-green-50 rounded-lg">
              <Wallet className="w-8 h-8 text-green-600 mx-auto mb-2" />
              <p className="text-sm text-green-800">
                Хэтэвч руу шилжиж байна...
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-green-600 to-green-700 text-white p-6">
              <div className="flex items-center justify-center gap-3 mb-2">
                <Wallet size={32} />
                <h2 className="text-2xl font-bold">Хэтэвч цэнэглэх</h2>
              </div>
              <p className="text-green-100 text-center">QPay Төлбөр</p>
              <p className="text-3xl font-bold mt-2 text-center">{formatPrice(amount)}</p>
            </div>
            ...
            <div className="p-8">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {urls.map((bank) => (
                      <a
                        ...
                        className="flex items-center gap-3 px-4 py-3 border-2 border-gray-200 rounded-lg hover:border-green-500 hover:bg-green-50 transition-colors"
                      >
                        {bank.logo && (
                          <img 
                            src={bank.logo} 
              ...
              <button
                ...
                disabled={checking}
                className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-white border-2 border-green-600 text-green-600 rounded-lg hover:bg-green-50 font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
        ...
        <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-6">
          ...
          <ol className="space-y-2 text-sm text-green-800">
            ...
            <li className="flex items-start gap-2">
              ...
              <span>Хэтэвчний үлдэгдэл автоматаар нэмэгдэнэ</span>
            </li>
          </ol>
        </div>
      </div>
